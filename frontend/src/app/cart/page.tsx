          ...
          {cartItems.length === 0 ? (
            <Card className="bg-[#1E1E1E] border-[#2A2A2A] shadow-sm">
              <CardContent className="py-16 text-center">
                ...
                <h2 className="text-2xl font-medium text-[#F5F5F5] mb-3">Корзина пуста</h2>
                <p className="text-[#A0A0A0] mb-8">
                  Добавьте товары из каталога для оформления заказа
                </p>
                <Link href="/catalog">
                  <Button size="lg" className="gap-2 bg-gold-500 hover:bg-gold-600">
                    Перейти в каталог
                  ...
                  <Card key={item.id} className="bg-[#1E1E1E] border-[#2A2A2A] shadow-sm overflow-hidden">
                    <CardContent className="p-4 flex gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <Link 
                          href={`/product/${item.product?.slug}`}
                          className="font-medium text-[#F5F5F5] hover:text-gold-500 line-clamp-2 transition-colors"
                        >
                          {item.product?.name}
                        </Link>
                        {item.product?.brand && (
                      ...
                      <div className="flex flex-col items-end justify-between">
                        ...
                        <div className="flex items-center bg-[#252525] rounded-lg">
                          ...
                          <span className="w-10 text-center font-medium text-[#F5F5F5]">{item.quantity}</span>
                          <button
                ...
                <Card className="bg-[#1E1E1E] border-[#2A2A2A] shadow-sm sticky top-24">
                  ...
                  <CardContent>
                    ...
                    <div className="flex justify-between text-xl font-bold mb-6">
                      <span className="text-[#F5F5F5]">К оплате</span>
                      <span className="text-gold-500">{formatPrice(totalAmount)}</span>
                    </div>

                    <Link href="/checkout" className="block">
                      <Button size="lg" className="w-full gap-2 bg-gold-500 hover:bg-gold-600 shadow-lg shadow-gold-500/25">
                        Оформить заказ
                    ...
                    <Link
                      href="/catalog"
                      className="block text-center text-gold-500 hover:text-gold-600 mt-4 transition-colors"
                    >
                      Продолжить покупки
                    </Link>
